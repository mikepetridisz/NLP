"""
Ez mukodik. Kiszedtem a Keywords column-ot, mert rengeteg request-et kuldott a HackerNews website-ra a URL-ek scrapelese.
A problema, hogy mikozben teszteltem sokszor futtattam a kodot es "Permanent Block" -ot kaptam.
=> urllib.error.HTTPError: HTTP Error 403: Forbidden) error
Mod_security / similar server security feature blocks known spider/bot user agents
-
Ez a verzio mukodik, de addig is dolgozok a megoldason, talalok ra megoldast. 
"""
...
class HackerNewsScraper:
    ...
    def print_stories(self):
        ...
        pdf = pd.DataFrame(data=(self._stories),
                           columns=['title', 'uri', 'author', 'points', 'comments', 'rank']) # 'keywords' taken out
...
def parse_arguments():
    ...
    parser.add_argument('--posts', '-p', metavar='n', type=int, default=50, help='number of posts (max 100)')
